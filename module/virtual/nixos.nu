...
export def download [--minimal] {
  if $minimal {
    ...
    if not ($filename | path exists) {
      https download https://channels.nixos.org/nixos-23.11/latest-nixos-minimal-x86_64-linux.iso -o $filename
    }
  } else {
    let filename = (dir | path join nixos-gnome-x86_64-linux.iso)
    if not ($filename | path exists) {
      https download https://channels.nixos.org/nixos-23.11/latest-nixos-gnome-x86_64-linux.iso -o $filename
    }
  }
}
